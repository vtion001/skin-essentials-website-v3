                                  ...
                                  <TableBody>
                                    {pageItems.map(a => (
                                      <TableRow key={a.id} className="group border-b border-[#E2D1C3]/10 hover:bg-[#FDFCFB] transition-colors cursor-default">
                                        ...
                                        <TableCell>
                                          <Badge className={`text-[10px] font-bold uppercase tracking-widest py-0.5 px-3 rounded-full border shadow-none ${a.status === 'completed' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                  ...
                  <TabsContent value="staff" className="space-y-8">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        <Button variant="outline" className="h-9 whitespace-nowrap" onClick={() => setPrivacyMode(prev => !prev)}>
                                ...
                                <TableBody>
                                  {rows.map((r, i) => (
                                    <TableRow key={i} className="group border-b border-[#E2D1C3]/10 hover:bg-[#FDFCFB] transition-colors cursor-default">
                                      ...
                                      <TableCell>
                                        <div className="text-[11px] font-bold text-[#1A1A1A] truncate max-w-[12rem]">{privacyMode ? maskName(r.clientName) : r.clientName}</div>
                                      </TableCell>
                                      <TableCell>
                                        ...
                                        {[...info.clients.entries()].map(([clientName, total]) => (
                                          <TableRow key={clientName} className="group border-b border-[#E2D1C3]/10 hover:bg-[#FDFCFB] transition-colors cursor-default">
                                            <TableCell className="py-2.5">
                                              <div className="text-[10px] font-bold text-[#1A1A1A] truncate max-w-[16rem]">{privacyMode ? maskName(clientName) : clientName}</div>
                                            </TableCell>
                                            <TableCell className="text-right text-[10px] font-bold text-[#1A1A1A]">₱{total.toLocaleString()}</TableCell>
                                            <TableCell className="text-right text-[10px] font-bold text-[#8B735B]">₱{(paymentsByClient.get(clientName) || 0).toLocaleString()}</TableCell>
                                            <TableCell className={`text-right text-[10px] font-bold ${(((paymentsByClient.get(clientName) || 0) - total) || 0) < 0 ? 'text-rose-600' : 'text-emerald-600'}`}>
                                              ₱{(((paymentsByClient.get(clientName) || 0) - total) || 0).toLocaleString()}
                                            </TableCell>
                                          </TableRow>
                                        ))}
                                      </TableBody>
                                ...
                                <TableRow key={s.id} className="group border-b border-[#E2D1C3]/10 hover:bg-[#FDFCFB] transition-colors cursor-default">
                                  ...
                                  <TableCell>
                                    <div className="text-[11px] font-bold text-[#1A1A1A]">
                                      {privacyMode ? (s.licenseNumber ? '••••••••' : '-') :
                                        (typeof s.licenseNumber === 'string' && s.licenseNumber.includes('"iv":') ? '••••••••' : (s.licenseNumber || '-'))}
                                    </div>
                                  </TableCell>
                                  <TableCell>
                                    <div className="text-[11px] font-bold text-[#1A1A1A]">{new Date(s.hireDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</div>
                                  </TableCell>
                                  <TableCell>
                                    <Badge className={`text-[10px] font-bold uppercase tracking-widest py-0.5 px-3 rounded-full border shadow-none ${s.status === 'active' ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-600 border-rose-100'}`}>
                                      {s.status}
                                    </Badge>
                                  </TableCell>
                                  <TableCell>
                                    {Array.isArray(s.treatments) && s.treatments.length > 0 ? (
                                      <Table className="bg-transparent">
                                        ...
                                        <TableBody>
                                          {s.treatments.slice(0, 3).map((t, i) => (
                                            <TableRow key={i} className="border-none">
                                              <TableCell className="py-1 text-[9px] font-bold text-[#1A1A1A] truncate max-w-[6rem]">{t.procedure}</TableCell>
                                              <TableCell className="py-1 text-[9px] text-[#8B735B] truncate max-w-[6rem]">{privacyMode ? maskName(t.clientName || '') : (t.clientName || '-')}</TableCell>
                                  ...
                                  <TableCell>
                                    <div className="flex items-center gap-1 justify-end">
                                      <Button size="icon" variant="ghost" onClick={() => {
                                        ...
                                        setStaffTreatmentForm([...(s.treatments || []).map((t: any) => ({
                                          ...t,
                                      ...
                                      }} className="h-8 w-8 rounded-lg text-emerald-600 hover:bg-emerald-50" title="Treatment Tracking">
                                        <Plus className="w-3.5 h-3.5" />
                                      </Button>
                                      <Button size="icon" variant="ghost" onClick={() => openStaffModal(s)} className="h-8 w-8 rounded-lg text-[#8B735B] hover:bg-[#E2D1C3]/20">
                                        <Edit className="w-3.5 h-3.5" />
                                      </Button>
                                      <Button size="icon" variant="ghost" onClick={() => { setStaffPreviewTarget(s); setIsStaffPreviewOpen(true) }} className="h-8 w-8 rounded-lg text-[#8B735B] hover:bg-[#E2D1C3]/20">
                                ...
                                <TableBody>
                                  {(Array.isArray(staffPreviewTarget.treatments) ? staffPreviewTarget.treatments : []).map((t, i) => (
                                    <TableRow key={i} className="group border-b border-[#E2D1C3]/10 hover:bg-[#FDFCFB] transition-colors cursor-default">
                                      ...
                                      <TableCell>
                                        <div className="text-[10px] text-[#8B735B] font-bold truncate max-w-[14rem]">{privacyMode ? maskName(t.clientName || '') : (t.clientName || '-')}</div>
                                      </TableCell>
                    ...
                    <Dialog open={isStaffTreatmentQuickOpen} onOpenChange={setIsStaffTreatmentQuickOpen}>
                      <DialogContent className="max-w-md sm:max-w-xl max-h-[70vh] overflow-y-auto p-4 sm:p-4">
                        ...
                        <div className="space-y-2">
                          {staffTreatmentForm.map((t, idx) => (
                            <div key={idx} className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-start">
                              ...
                              <div className="sm:col-span-3 space-y-1.5">
                                ...
                                <Select value={t?.clientId || ''} onValueChange={(value) => {
                                  const client = clients.find(c => c.id === value);
                                  setStaffTreatmentForm(prev => prev.map((x, i) => i === idx ? { ...x, clientId: value, clientName: client ? `${client.firstName} ${client.lastName}`.trim() : '' } : x))
                                }}>
                                  ...
                                  <SelectContent>
                                    {clients.map(c => (
                                      <SelectItem key={c.id} value={c.id}>{`${c.firstName} ${c.lastName}`.trim()}</SelectItem>
                                    ))}
                                  </SelectContent>
                                </Select>
                              </div>
                          ...
                          <Button type="button" variant="outline" onClick={() => setStaffTreatmentForm(prev => ([...prev, { date: new Date().toISOString().split('T')[0], procedure: '', clientName: '', clientId: '', total: 0 }]))}>
                            <Plus className="w-4 h-4 mr-2" />
                            Add Treatment
                          </Button>
                        </div>
                        <div className="flex justify-end gap-2 mt-4">
                          ...
                          <Button type="button" onClick={async () => {
                            ...
                            try {
                              // Hook to the new treatments table API
                              ...
                              console.error(e)
                              // fall back to staff object update if table sync fails
